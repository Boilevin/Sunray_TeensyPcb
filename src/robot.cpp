...
// give Arduino IDE users some time to open serial console to actually see very first console messages
  ...
  // bber watchdogEnable(10000L);   // 10 seconds
...
bool detectObstacle()
{
  if (!((robotShouldMoveForward()) || (robotShouldRotate())))
    return false;
  if (TOF_ENABLE)
  {
...
bool detectObstacleRotation()
{
  ...
  if (!OBSTACLE_DETECTION_ROTATION)
    return false;
  if (millis() > angularMotionStartTime + 15000)
  { // too long rotation time (timeout), e.g. due to obstacle
  ...
  if (imuDriver.imuFound)
  {
    ...
    { // yaw speed difference between wheels and IMU more than 8 degree/s, e.g. due to obstacle
...
void run()
{
  ...
  {
    gps.solutionTimeout = millis() + 1000;
  }
