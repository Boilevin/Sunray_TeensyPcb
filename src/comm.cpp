...
void cmdPosMode(){
  ...
  for (unsigned int idx=0; idx < cmd.length(); idx++){
    ...
    if ((ch == ',') || (idx == cmd.length()-1)){
      int intValue = cmd.substring(lastCommaIdx+1, ch==',' ? idx : idx+1).toInt();
      #ifdef  __IMXRT1062__  //teensy 4 don't have the .todouble in string class
        double doubleValue = double(cmd.substring(lastCommaIdx+1, ch==',' ? idx : idx+1).toFloat());
      #else 
...
if (cmd[3] == 'R'){       // Handling for apps to read out to actual used perimeter, exclusion, docking, waypoint values from mower to be able to use the same configuration in several app devices without needed internet server copy process
  ...
  if (cmd[3] == 'Y') {
    if (cmd.length() <= 4){
      ...
      if (cmd[4] == '4') robotDriver.mapUploadingActive=true;   // stop reading gps while map uploading from pi
